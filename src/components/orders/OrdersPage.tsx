...
export const OrdersPage: React.FC = () => {
  const { orders, navigateTo, addToCart, addToast, addProductComment } = useStore();
  ...
  const handleReviewSubmit = (e: React.FormEvent) => {
    ...
    addProductComment(reviewProductId, {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <SheetContent side="right" className="w-full sm:max-w-xl p-0 flex flex-col h-full bg-white shadow-2xl border-l border-slate-200">
            {selectedOrder && (
              <div className="flex flex-col h-full overflow-hidden">
                ...
                {/* 1. Sheet Header */}
                <SheetHeader className="p-6 pb-4 border-b border-slate-100 bg-white sticky top-0 z-10 space-y-1 relative">
                  <button
                    onClick={() => setIsSheetOpen(false)}
                    className="absolute top-5 right-5 w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 transition-colors z-20"
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-[#fafaf9]">
                  ...
                  {/* Delivered Product Review Notification */}
                  {selectedOrder.status === 'delivered' && (
                    <div className="p-4 rounded-2xl bg-indigo-50 border border-indigo-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center flex-shrink-0">
                          <Star className="w-5 h-5 fill-current" />
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-indigo-950">Review Your Items</h4>
                          <p className="text-xs text-indigo-700 mt-0.5">Your order has been delivered. Share your experience with the community!</p>
                        </div>
                      </div>
                      <Button 
                        onClick={() => {
                          const firstItemId = selectedOrder.items[0]?.productId;
                          if (firstItemId) {
                            openReviewModal(firstItemId);
                          }
                        }}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-xs rounded-full text-xs font-bold px-4 h-9"
                      >
                        Write Review
                      </Button>
                    </div>
                  ...
                  <div className="p-5 sm:p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs space-y-4">
                    <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-200">
                        {selectedOrder.deliveryOption.name}
                      </span>
                    </div>

                    <div className="space-y-0 pl-1 pt-1 relative">
                      {selectedOrder.trackingSteps.map((step, idx) => {
                        const isLast = idx === selectedOrder.trackingSteps.length - 1;
                        return (
                          <div key={idx} className="flex items-start gap-4 relative group pb-6 last:pb-0">
                            {!isLast && (
                              <div
                                className={cn(
                                  "absolute left-3 top-6 bottom-0 w-0.5 -translate-x-1/2 rounded-full",
                            ...
                            <div
                              className={cn(
                                "relative w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 z-10 transition-all",
                                step.current
                                  ? "bg-amber-400 text-slate-950 ring-4 ring-amber-400/20 shadow-xs mt-1"
                                  : step.completed
                                  ? "bg-amber-400 text-slate-950 mt-1"
                                  : "bg-white border-2 border-slate-200 text-slate-300 mt-1"
                              )}
                            >
                              {step.completed ? (
                            ...
                            <div className="flex-1 min-w-0 bg-slate-50 hover:bg-slate-100 transition-colors p-3.5 rounded-2xl border border-slate-100">
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-xs">
                                <span className={cn("font-bold", step.completed || step.current ? "text-slate-950" : "text-slate-500")}>
                                  {step.title}
                                </span>
                                {step.date && (
                                  <div className="flex items-center gap-1.5 text-[10px] font-mono text-slate-500 bg-white px-2 py-0.5 rounded-md border border-slate-200 shadow-sm">
                                    {step.date}
                                  </div>
                                )}
                              </div>
        ...
        {reviewModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setReviewModalOpen(false)} />
            <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
              <div className="p-6 sm:p-8">
                ...
                <p className="text-xs text-slate-500 mb-6">
                  Tell us about your experience with this item.
                </p>
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-5">
                  <div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-bold text-slate-800 ml-2">{reviewRating}.0 Stars</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe what you liked or didn't like..."
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <Button
                      ...
                      className="flex-1 rounded-2xl h-11 bg-slate-950 text-white hover:bg-slate-800"
                    >
                      Submit Review
                    </Button>
                  </div>
